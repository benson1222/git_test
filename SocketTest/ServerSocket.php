...
class WebSocket {
    const LOG_PATH = 'E:/PhpDeveloper/Demo/SocketDemo';
    ...
    public function __construct($host, $port,$_delegate) {
        $this->delegate = new $_delegate;
        try {
            ...
            // listen函数使用主动连接套接口变为被连接套接口，使得一个进程可以接受其它进程的请求，从而成为一个服务器进程。在TCP服务器编程中listen函数把进程变为一个服务器，并指定相应的套接字变为被动连接,其中的能存储的请求不明的socket数目。后面的参数则是自定义的待处理socket的最大数目，并发高的情况下，这个值可以设置大一点，虽然它也受系统环境的约束。
        ...
        $pid = get_current_user();//posix_getpid();
    ...
    private function doServer() {
        ...
        foreach ($sockets as $socket) {
            ...
            if ($socket == $this->master) {
                ...
                // socket_accept() 接受 请求 “正在 listen 的 socket（像我们的服务器 socket ）” 的连接, 并返回一个客户端 socket, 错误时返回 false;
...
//        socket_write($socket, $msg, strlen($msg));
     ...
     * @param $data
     */
    private function readData($socket, $recv_msg) {
        $this->delegate->readData($socket,$recv_msg);
    }
}
